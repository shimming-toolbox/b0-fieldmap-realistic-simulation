...
SUBJECTS=$(ls "$BIDS_DIR/data/b0shim" | grep sub-)
...
do
    echo "st_b0shim dynamic --fmap ${BIDS_DIR}/data/b0shim/$subject/${subject}_T1w_fmap_b0-sim_head.nii.gz --anat ${BIDS_DIR}/data/b0shim/$subject/${subject}_T1w_head.nii.gz --mask ${BIDS_DIR}/data/b0shim/$subject/${subject}_mask_brain.nii.gz --scanner-coil-order "0,1,2" -o ${BIDS_DIR}/data/b0shim/$subject/output-paper-step1/ --slices volume" >> run_4_shim.sh
    echo "st_b0shim dynamic --fmap ${BIDS_DIR}/data/b0shim/$subject/output-paper-step1/fieldmap_calculated_shim.nii.gz --anat ${BIDS_DIR}/data/b0shim/$subject/${subject}_T1w_head.nii.gz --mask ${BIDS_DIR}/data/b0shim/$subject/${subject}_mask_brain.nii.gz --scanner-coil-order "0,1" -o ${BIDS_DIR}/data/b0shim/$subject/output-paper-step2/ --slices descending --slice-factor 1" >> run_4_shim.sh
done
